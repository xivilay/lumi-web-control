...
function set_scale(name) {
    var scales = {
        ...
        'arabic (a)':           [0x10, 0x60, 0x22, 0x03, 0x00, 0x00, 0x00, 0x00], // arabic (a)
...
function set_key(key) {
    const keys = {
        ...
        'C#': [0x10, 0x30, 0x23, 0x00, 0x00, 0x00, 0x00, 0x00],
        'D':  [0x10, 0x30, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00],
        'D#': [0x10, 0x30, 0x63, 0x00, 0x00, 0x00, 0x00, 0x00],
        'E':  [0x10, 0x30, 0x03, 0x01, 0x00, 0x00, 0x00, 0x00],
        'F':  [0x10, 0x30, 0x23, 0x01, 0x00, 0x00, 0x00, 0x00],
        'F#': [0x10, 0x30, 0x43, 0x01, 0x00, 0x00, 0x00, 0x00],
        'G':  [0x10, 0x30, 0x63, 0x01, 0x00, 0x00, 0x00, 0x00],
        'G#': [0x10, 0x30, 0x03, 0x02, 0x00, 0x00, 0x00, 0x00],
        'A':  [0x10, 0x30, 0x23, 0x02, 0x00, 0x00, 0x00, 0x00],
        'A#': [0x10, 0x30, 0x43, 0x02, 0x00, 0x00, 0x00, 0x00],
        'B':  [0x10, 0x30, 0x63, 0x02, 0x00, 0x00, 0x00, 0x00],
    };
    if (key in keys) {
        log('SEND ' + key)
        send_sysex(keys[key]);
...
function set_pressure_tracking_mode(key) {
    const values = {
    ...
    if (key in values) {
        log('SEND ' + key)
        send_sysex(values[key]);
    } else {
        log("unknown pressure tracking mode " + key);
    }
}
